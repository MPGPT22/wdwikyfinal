        ...
        <div class="col-md-12">
          ...
          <div class="row">
            @foreach ($list as $item)
              <div class="col-lg-6 col-md-12 divide-rows">
                  <div class="row-left">
                    @if ($item->public == 1)
                      <a href="{{ route('states.set.not.public', $item->id) }}" class="title"><i class="far fa-eye fa-lg"></i></a>
                    @else
                      <a href="{{ route('states.set.public', $item->id) }}" class="title"><i class="far fa-eye fa-lg" style="color:#bbb"></i></a>
                    @endif
                  </div>
                  <a class="a_no_style" href="{{route('states.show', $item->id)}}">
                  <div class="row-center">
                    {{$item->name}}
                  </div>
                  </a>
                  <div class=" row-right">
                    {!! Form::open(['route'=>['states.destroy', $item->id], 'method' => 'DELETE', 'class' => 'btn100']) !!}
                    <div class="btn-group btn100">
                      <a href="{{ route('states.edit', $item->id) }}" class='btn btn-info btn-colormode btn-50'><i style="font-size: 20px;" class="fa fa-cog"></i></a>
          ...
          @else
            ...
            <div class="col-md-12 title">
              <h3>We don't have any states created.</h3>
              <br>
              <a class="btn btn-primary" href="{{ route('states.create') }}">Go To Create</a>
            </div>
              
            @else
            <div class="col-md-12 title">
              <h3>We don't have any states created. Wait until someone creates one.</h3>
